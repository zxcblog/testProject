...
func (c *categoryCatch) GetCategoryByPath(path string) *models.Category {
	if path == "" {
		return nil
	}
...
// SetCategory 设置分类缓存
func (c *categoryCatch) SetCategory(category *models.Category) (bool, error) {
...
// DelCategory 删除缓存
func (c *categoryCatch) DelCategory(key string) (int64, error) {
	field := c.Key + key + "*"
	fmt.Println(field)
	keys, err := global.Redis.Keys(c.ctx, field).Result()
	if err != nil {
